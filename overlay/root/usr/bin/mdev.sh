...
mdev_mount()
{
	...
	if ! mount -t auto -o noatime,iocharset=utf8,check=r,shortname=mixed "/dev/$1" "${mnt}/$1"; then
		mount.exfat -o noatime,iocharset=utf8,check=r,shortname=mixed "/dev/$1" "${mnt}/$1"; e=$?
		if [ $e -gt 0 ]; then
